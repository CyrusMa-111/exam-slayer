#!/usr/bin/env python3
"""Build a time-boxed Exam Slayer pack from 考试画像.json."""
...
def build_slayer_plan(profile: dict, out_dir: Path, time_budget: str, target: str) -> None:
    ...
    (out_dir / SLAYER_PLAN_MD).write_text("\n".join(lines), encoding="utf-8")
...
def build_quick_review(profile: dict, out_dir: Path, time_budget: str) -> None:
    ...
    for i, topic in enumerate(topics, 1):
        ...
        lines.extend([
            ...
            "",
            "$$",
            "\\text{在这里填写公式，例如 } H(X) = -\\sum_i p_i \\log_2 p_i",
            "$$",
            "",
            "- 变量含义：",
            "  - $p_i$：",
        ...
        lines.append("")
    (out_dir / QUICK_REVIEW_MD).write_text("\n".join(lines), encoding="utf-8")
...
def build_practice(profile: dict, out_dir: Path, time_budget: str) -> None:
    ...
    for i, topic in enumerate(topics, 1):
        ...
        answers.append("")
    (out_dir / PRACTICE_SET_MD).write_text("\n".join(lines), encoding="utf-8")
    (out_dir / PRACTICE_ANSWERS_MD).write_text("\n".join(answers), encoding="utf-8")


def build_flashcards(profile: dict, out_dir: Path, time_budget: str) -> None:
    cfg = TIME_LIMITS[time_budget]
    topics = profile.get("topics", [])[: cfg["top"]]
    with (out_dir / FLASHCARDS_CSV).open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
...
def build_risk_report(profile: dict, out_dir: Path) -> None:
    ...
    (out_dir / RISK_REPORT_MD).write_text("\n".join(lines), encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description="Build an Exam Slayer pack from 考试画像.json.")
    parser.add_argument("exam_profile", help="Path to 考试画像.json generated by analyze_exam_frequency.py.")
